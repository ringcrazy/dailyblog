...
* 两个值： value和reason ,value表示value成功后，向下传递的值， reason为失败后，向下传递的值,
* 状态一旦改变，就不会再变。
* 创造promise实例后，它会立即执行
* 每个promise的实例上都有一个then方法，then方法有两个回调函数,执行then方法后，根据上一个promise实例状态的值，选择不同的回调函数进行执行。
* then方法中返回的还是一个Promise,这样可以支持链式调用
